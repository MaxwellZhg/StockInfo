...
class SettingFragment :
    AbsSwipeBackNetFragment<FragmentSettingBinding, SettingViewModel, SettingView, SettingPresenter>(),
    SettingView {
    ...
    companion object {
        fun newInstance(type: Int, title: String): SettingFragment {
            val fragment = SettingFragment()
            if (type != 0) {
                val bundle = Bundle()
                bundle.putInt("type", type)
                bundle.putString("tips", title)
