 ...
 *    desc   :
 */
class CapitalFlowNumPopWindow(contentView: View, num: Int, width: Int, height: Int) :
    ...
    init {
        ...
        contentView.findViewById<View>(R.id.layout_20).setOnClickListener(this)
        contentView.findViewById<View>(R.id.layout_60).setOnClickListener(this)
        var id = when (num) {
            5 -> {
                R.id.tv_5
            }
    ...
    companion object {
        ...
        fun create(context: Context, mode: Int, callback: OnSelectCallBack): CapitalFlowNumPopWindow {
            val popupWindowView =
                View.inflate(context, R.layout.pop_capital_flow_num, null)
            val popupWindow = CapitalFlowNumPopWindow(
                popupWindowView,
                mode,
