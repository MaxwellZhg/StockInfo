...
import com.zhuorui.securities.alioss.Config;
...
public class OssService {
    ...
    private String url = "/as_common/api/common/sts/token";

    private final Set VIDEO_SUFFIX;

    public String createUpImageName(String suffix) {
        return createName("images", suffix);
    }

    public String createUpVideoName(String suffix) {
        return createName("video", suffix);
    }

    public String createName(String type, String suffix) {
        long timeMillis = TimeZoneUtil.currentTimeMillis();
        String strRand = "";
        for (int i = 0; i < 4; i++) {
            strRand += String.valueOf((int) (Math.random() * 10));
        }
        return String.format("%s/%s/%s%s%s", type, TimeZoneUtil.timeFormat(timeMillis, "yyyy/MM/dd"), timeMillis, strRand, suffix);
    }

    public OssService(Context applicationContext) {
        VIDEO_SUFFIX = new HashSet();
            ...
            public OSSFederationToken getFederationToken() throws ClientException {
                ...
                try {
                    ...
                    if (TextUtils.equals(code, "000000")) {
                        ...
                        JSONObject bucket = data.getJSONObject("bucketName");
                        mBucket = data.getString("bucketName");
                        mEndpoint = data.getString("endpoint");
                        return new OSSFederationToken(ak, sk, token, expiration);
                    } else {
                        String msg = jsonObjs.getString("msg");
        ...
        mOss = new OSSClient(applicationContext, mEndpoint, credentialProvider);
    }

    public io.reactivex.Observable<String> getPutObjectObservable(final String uploadFilePath) {
        String suffix = uploadFilePath.substring(uploadFilePath.lastIndexOf("."));
        return getPutObjectObservable(VIDEO_SUFFIX.contains(suffix) ? createUpVideoName(suffix) : createUpImageName(suffix));
    }

    public io.reactivex.Observable<String> getPutObjectObservable(final String objectName, final String uploadFilePath) {
        return io.reactivex.Observable.create(new ObservableOnSubscribe<String>() {
            @Override
            public void subscribe(ObservableEmitter emitter) throws Exception {
                PutObjectRequest put = new PutObjectRequest(mBucket, objectName, uploadFilePath);
                ...
                try {
                    ...
                    emitter.onNext(putResult.getETag());
                    emitter.onComplete();
    ...
    public io.reactivex.Observable getPutObjectObservable(final String objectName, final byte[] uploadData) {
        return io.reactivex.Observable.create(new ObservableOnSubscribe() {
            @Override
            public void subscribe(ObservableEmitter emitter) throws Exception {
                PutObjectRequest put = new PutObjectRequest(mBucket, objectName, uploadData);
                put.setProgressCallback(new OSSProgressCallback<PutObjectRequest>() {
                    @Override
                    public void onProgress(PutObjectRequest request, long currentSize, long totalSize) {
